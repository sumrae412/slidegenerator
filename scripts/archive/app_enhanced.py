...
class ClaudeCodeAgent:
    def __init__(self):
        # Try Claude Code specific key first, fallback to Anthropic key
        claude_code_key = os.getenv("CLAUDE_CODE_API_KEY")
        anthropic_key = os.getenv("ANTHROPIC_API_KEY")
        
        if claude_code_key:
            self.options = ClaudeCodeOptions()
            # Set API key if the SDK supports it
        elif anthropic_key:
            self.options = ClaudeCodeOptions()
        else:
            st.error("Neither CLAUDE_CODE_API_KEY nor ANTHROPIC_API_KEY found")
            self.options = None
...
class WorkflowOrchestrator:
    ...
    async def execute_collaborative_workflow(self, prompt: str, max_iterations: int = 2) -> List[WorkflowResult]:
        ...
        # Stage 2: Initial Code Generation with Claude Code
        st.info("⚡ Stage 2: Generating initial code implementation...")
        initial_code = await self.claude_code.generate_code(prompt, planning)
        if initial_code:
            self.results.append(WorkflowResult(
...
def main():
    st.set_page_config(
        page_title="Multi-Agent Code Collaboration",
        page_icon="🤖",
        layout="wide"
    )
    
    st.title("🤖 Multi-Agent Code Collaboration")
    st.subheader("Claude + Claude Code + ChatGPT Working Together")
    ...
    with st.sidebar:
        ...
        st.code("""
# Option 1: Use same key for both (most common)
ANTHROPIC_API_KEY=your_anthropic_api_key
OPENAI_API_KEY=your_openai_api_key

# Option 2: Separate keys (if needed)
ANTHROPIC_API_KEY=your_anthropic_api_key
...
OPENAI_API_KEY=your_openai_api_key
        ...
        2. **Code Generation** - Claude Code creates initial implementation
    ...
    if start_workflow and prompt.strip():
        ...
        if results:
            st.success("🎉 Collaborative workflow completed!")
    ...
    with st.expander("ℹ️ How It Works"):
        ...
        This app orchestrates three AI agents to create the best possible code:
        
        **🧠 Claude** - Strategic thinking and refinement
        - Analyzes requirements and creates implementation plans
        - Refines code based on feedback from reviews
        - Focuses on architecture and best practices
        
        **⚡ Claude Code** - Practical implementation
        ...
        **🔍 ChatGPT** - Quality assurance and validation
        - Performs comprehensive code reviews
        - Identifies potential issues and improvements
        - Provides final validation and recommendations
        
        ### 🔄 Iterative Improvement
        The workflow includes multiple refinement cycles where each agent's strengths complement the others, resulting in higher quality code than any single agent could produce alone.
        """)
