...
class ProgressTracker:
    ...
    def update(self, stage_num: int, message: str):
        """Update progress bar and status message"""
        if self.progress_bar and self.status_text:
            progress = stage_num / self.total_stages
            self.progress_bar.progress(progress)
            self.status_text.info(f"Stage {stage_num}/{self.total_stages}: {message}")
    ...
    def complete(self):
        ...
        if self.progress_bar and self.status_text:
            ...
            self.status_text.success("🎉 Workflow completed successfully!")
...
class ClaudeAgent:
    ...
    async def analyze_requirements(self, prompt: str) -> Optional[str]:
        ...
        try:
            with st.spinner("🎯 Claude is analyzing requirements..."):
...
Please provide complete, working code that can be run immediately."""
    ...
    async def refine_code(self, code: str, feedback: str) -> Optional[str]:
        ...
        try:
            with st.spinner("🔄 Claude is refining the code..."):
                response = await asyncio.to_thread(
                    self.client.messages.create,
                    model="claude-3-5-sonnet-20241022",
                    max_tokens=3000,
...
class ChatGPTAgent:
    ...
    def _get_optimal_max_tokens(self, model: str, requested_tokens: int) -> int:
        ...
        # Ensure requested_tokens is an integer
        if not isinstance(requested_tokens, int):
            requested_tokens = int(requested_tokens)
        
        model_limits = {
            "gpt-5": min(int(requested_tokens * 2), 4000),
            "gpt-4o": min(int(requested_tokens * 1.5), 4000),
            "gpt-4": min(requested_tokens, 4000),
            "gpt-4-turbo": min(int(requested_tokens * 1.3), 4000),
            "gpt-3.5-turbo": min(requested_tokens, 4000),
            "gpt-3.5-turbo-16k": min(requested_tokens, 4000)
        }
        return int(model_limits.get(model, requested_tokens))
    
    def _make_api_call_with_retry(self, messages: list, max_tokens: int, max_retries: int = 3):
        """Make API call with retry logic and progress updates"""
        if not self.client:
            return None
        ...
        for model in models_to_try:
            for retry in range(max_retries):
                try:
                    ...
                    if retry > 0:
                        st.info(f"🔄 Retry {retry + 1} with {model}...")
                    
                    response = self.client.chat.completions.create(
                        model=model,
                        messages=messages,
                        max_tokens=optimal_tokens,
                        timeout=30  # Reduced to 30 second timeout
                    )
                    ...
                    if "does not exist" in error_msg or "not found" in error_msg:
                        ...
                        if retry < max_retries - 1:
                            ...
                            st.warning(f"⚠️ Rate limit persists for {model}, trying next model...")
                            break
                        ...
                        if retry < max_retries - 1:
                            st.warning(f"⚠️ API error: {str(e)[:100]}... Retrying...")
                            time.sleep(2)
                            continue
                        else:
                            break
    ...
    def review_build_plan(self, plan: str, original_prompt: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            with st.spinner("🔍 ChatGPT is reviewing the build plan..."):
...
Provide actionable suggestions and highlight any critical gaps."""
    ...
    def comprehensive_review(self, code: str, original_prompt: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            with st.spinner("🔍 ChatGPT is performing comprehensive code review..."):
                # Add a progress placeholder to keep connection alive
                progress_placeholder = st.empty()
                progress_placeholder.info("⏳ Analyzing code structure...")
...
Format your response with clear sections and be specific about issues and solutions."""
    ...
    def final_validation(self, refined_code: str, original_prompt: str) -> Optional[str]:
        ...
        try:
            with st.spinner("✅ ChatGPT is performing final validation..."):
                response = self._make_api_call_with_retry(
                    messages=[{
                        "role": "user",
...
Keep response concise but thorough."""
    ...
    def _extract_rating(self, review_text: str) -> str:
        """Extract code quality rating from review text"""
...
## Features
- Generated using AI collaboration (Claude + ChatGPT/GPT-5)
...
class WorkflowOrchestrator:
    ...
    async def execute_collaborative_workflow(self, prompt: str, project_name: str, max_iterations: int = 2) -> Tuple[List[WorkflowResult], List[ProjectFile]]:
        ...
        try:
            ...
            self.progress_tracker.update(1, "Claude analyzing requirements and creating plan")
            ...
            self.progress_tracker.update(2, "ChatGPT reviewing build plan")
            ...
            self.progress_tracker.update(3, "Claude generating code implementation")
            ...
            self.progress_tracker.update(4, "ChatGPT performing comprehensive code review")
            ...
            if review_result:
                ...
                for iteration in range(max_iterations):
                    self.progress_tracker.update(5, f"Claude refining code (iteration {iteration + 1}/{max_iterations})")
            ...
            self.progress_tracker.update(6, "ChatGPT performing final validation")
            ...
            self.progress_tracker.update(7, "Generating complete project structure")
...
def main():
    st.set_page_config(
        page_title="Stable Multi-Agent Code Collaboration",
        page_icon="🤖",
        layout="wide"
    )
    
    st.title("🤖 Stable Multi-Agent Code Collaboration")
    st.subheader("Claude + GPT-5/ChatGPT → Reliable Project Generation")
    
    # Add connection status indicator
    connection_status = st.empty()
    connection_status.success("🟢 Connection Active")
    
    # Sidebar configuration
    with st.sidebar:
        st.header("⚙️ Workflow Configuration")
        max_iterations = st.slider("Max Refinement Iterations", 1, 3, 2)
...
OPENAI_API_KEY=your_openai_api_key
        ...
        st.header("🛡️ Stability Features")
        st.write("""
        - **Connection Monitoring** - Tracks app status
        - **Retry Logic** - Auto-retry failed API calls
        - **Progress Tracking** - Visual progress indicators
        - **Timeout Handling** - Prevents hanging connections
        - **Graceful Fallbacks** - Continues even if stages fail
        """)
    ...
    if start_workflow and prompt.strip() and project_name.strip():
        ...
        try:
            ...
            if results and any(r.stage != WorkflowStage.COMPLETE for r in results):
                st.success("🎉 Complete project generated successfully!")
                display_workflow_results(results, project_files)
                connection_status.success("🟢 Workflow Completed Successfully")
            else:
                st.error("❌ Workflow failed to produce results.")
                connection_status.error("🔴 Workflow Failed")
                
        except Exception as e:
            st.error(f"❌ Critical error: {str(e)}")
            connection_status.error("🔴 Connection Error - Please restart the app")
            st.info("💡 If this error persists, try restarting Streamlit: `streamlit run app_stable.py`")
